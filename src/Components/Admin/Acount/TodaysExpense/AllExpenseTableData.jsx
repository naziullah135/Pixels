...
const AllExpenseTableData = ({ refetch, item, day, overRefetch, accountRefetch }) => {
    ...
    return (
        ...
        <tr className="border-b border-opacity-20 border-gray-500 ">
            ...
            <td className="p-3 text-left text-sm">
                {<button onClick={() => setAddExpenseModalOpen(true)} disabled={item?.isPaid === "paid"} className={`${item?.isPaid === "paid" ? 'cursor-not-allowed bg-gray-200 text-gray-300' : 'bg-primary text-white'}  px-2 font-bold rounded-md flex items-center gap-1`}>Pay<Icon icon="zondicons:add-solid" /></button>}
                {/* {day === "all" && date} */}
            </td>
            <td className="p-3 text-left text-sm ">
...
export default AllExpenseTableData;